...
% 读取测试词汇ID
vocabulary_temp = importdata('input\sign_242.txt');
for i=1:242
    vocabulary(i) = str2double(vocabulary_temp{i,1}(2:5));
end
...
sample = 3;            % 隔n帧采样
thre = 880;         % score>thre 的视为有效
draw = 1;           % 1:显示视频。 0：不显示视频
windowSize = 30;    % 滑动窗口的大小
cutRegion = 4;
fidName = ['result\HMM_result' '_2SegModel_thre' num2str(thre) '_skip' ...
    num2str(sample) '_win' num2str(windowSize) '_Real209_242sign_BP2D_G0801_show.txt' ];
...
for groupID =  1:1
    ...
    shwoEnglish{6} = 'Groundtruth: (English: When are you free?)';
...
%         end
        ...
        for k=cutRegion:nframes-cutRegion
            ...
            if  mod(k,sample)==0 %k>windowSize/2 && k<nframes - windowSize/2 &&
                t = max(k - windowSize/2,cutRegion);
                t_= min(k + windowSize/2, nframes-cutRegion);
...
%                    predict_label_P1 = vocabulary(index_sort(1));
                   ...
                   if predict_label_P1~=236    %%%%%%%%%%%%% trick
                       showText_result1 = ['Sign: '...
                        ...
                        showSlideScore = num2str(score_sort(1));
